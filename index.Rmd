...
```{r include=FALSE}
fviz_nbclust(inputData, hcut,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F,hc_func = "diana")
```

```{r include=FALSE}
fviz_nbclust(inputData, hcut,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F,hc_func = "agnes")
```
...
```{r}
#Hacemos los clusters:
res.pam = pam(analisis,3,cluster.only = F)
res.agnes = hcut(analisis, k = 3,hc_func='agnes',hc_method = "ward.D")
res.diana = hcut(analisis, k = 3,hc_func='diana')
```
